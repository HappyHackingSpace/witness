...
export default defineBackground(() => {
  ...
  function cdp(tabId: number, method: string, params?: Record<string, unknown>): Promise<any> {
    ...
    try {
      ...
      if (fullpageScreenshot) {
        ...
        if (!screenshotDataUrl) {
          screenshotDataUrl = await chrome.tabs.captureVisibleTab({ format: "png", quality: 100 });
        }
      } else {
  ...
  async function handleMessage(message: { type: string; payload?: unknown }) {
    switch (message.type) {
      ...
      case "UPDATE_CAPTURE": {
        const { captureId: updateId, updates: rawUpdates } = message.payload as {
          captureId: string;
          updates: Record<string, unknown>;
        };
        // Whitelist allowed fields to prevent overwriting critical metadata
        const safeUpdates: Partial<{ tags: string[]; notes: string }> = {};
        if (Array.isArray(rawUpdates.tags)) safeUpdates.tags = rawUpdates.tags.map(String);
        if (typeof rawUpdates.notes === "string") safeUpdates.notes = rawUpdates.notes;
        await captureStore.updateCapture(updateId, safeUpdates);
        return { success: true };
      }
